...
namespace LFTC___LABORATOR_4
{
    ...
    public class Grammar
    {
        ...
        public void SolveInput(string inputSequence)
        {
            Stack<string> productionStack = new Stack<string>();
            Stack<string> inputStack = new Stack<string>();
            productionStack.Push("$");
            inputStack.Push("$");
            productionStack.Push(this.StartingSymbol);
            var firstSet = this.GetFirstUs();
            var followSet = this.GetFollowUs();

            var splittedSequence = inputSequence.Split(' ');
            splittedSequence.Reverse();

            foreach (var s in splittedSequence)
            {
                inputStack.Push(s);
            }
            ...
            {
                Tuple<string, string> pair = new Tuple<string, string>(inputStack.Peek(), productionStack.Peek());

                if (pair.Item1 == pair.Item1)
                {
                    Console.WriteLine($"POP {pair.Item1} {pair.Item2}");
                    inputStack.Pop();
                    productionStack.Pop();
                }
                else
                {
                    if (this.NonTerminals.Contains(pair.Item2))
                    {
                        if (firstSet[pair.Item2].Contains(pair.Item1))
                        {
                            var containingProduction = this.GetProductionContainingTerminal(pair);
                            if (containingProduction != null)
                            {
                                Console.WriteLine($"POP {pair.Item2} PUSH {containingProduction.RightHandSide}");
                                productionStack.Pop();
                                containingProduction.RightHandSide.Reverse();
                                containingProduction.RightHandSide.ForEach(p => productionStack.Push(p));
                            }
                            else
                            {
                                Console.WriteLine($"Error {pair.Item2} {pair.Item1}");
                            }
                        }
        ...
        private Production GetProductionContainingTerminal(Tuple<string, string> pair)
        {
            foreach (var production in this.Productions)
            {
                if (production.LeftHandSide == pair.Item2)
                {
                    if (production.RightHandSide.First() == pair.Item1)
                    {
                        return production;
                    }
                }
